...
class Auth {
    ...
    static function authorize(){
        ...
        $redirect_uri = get_site_url() . "?flowy_paywall_callback";
        $url = "${api_url}/login/oauth/authorize?response_type=code&client_id=${client_id}&redirect_uri=${redirect_uri}";
        wp_redirect($url);
        exit;
    }
    ...
    static function doTokenLogin($code){
        ...
        curl_setopt($ch, CURLOPT_URL, "${api_url}/login/oauth/token");
